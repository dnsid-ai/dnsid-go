...
- [**breaking**] feat: add the `config` package implementing SDK design 12 (configuration loading): `LoadEnvironment`, `LoadCliDirectory`, `Merge`, `Construct`, `IdentityManagerFromEnvironment`, `IdentityManagerFromDnsid`, and `RegistryClientFromEnvironment`. Loaders parse only; `dnsid.NewIdentityManager` applies every default. Removed `dnsid.ConfigFromEnv`, `dnsid.NewIdentityManagerFromDnsid`, and `dnsid.NewRegistryClientFromEnv`. Loaders no longer substitute a `noop:0` log reference or derive `statusUrl` from a registry/server URL; `LoadCliDirectory("")` reads `~/.dnsid` and never consults `DNSID_CONFIG_DIR`. New variables `DNSID_CAPABILITIES_URL`, `DNSID_LOG_POLICY_URL`, `DNSID_LOG_POLICY_FILE`, `DNSID_LOG_TRUST_PROFILE_FILE`, `DNSID_KEY_STORE`. The validate-domain example drops `DNSID_TRUST_PROFILE`, `DNSID_POLICY_URL`, and `DNSID_BUNDLE_VERIFIER_KEY`.
- [**breaking**] fix: replace `config.Dependencies.Options` with explicit `DNSResolver`, `HTTPSFetcher`, `IdentityCache`, and `HTTPClient` fields. `Construct` can now honor caller-supplied dependencies without fetching displaced log trust or reading displaced keys.
- [**breaking**] fix: keep `DNSID_API_KEY` out of `config.Loaded`; `RegistryClientFromEnvironment` reads the credential directly and explicit `WithAuthToken` options still win.
